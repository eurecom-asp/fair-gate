...
def accuracy(output, target, topk=(1,)):
	...
	return res
# ---------------------------------------------------------------------
# Public API aliases for legacy ECAPA utilities.
# ---------------------------------------------------------------------

def tune_threshold_from_score(scores, labels, target_fa, target_fr=None):
    """Readable alias for tuneThresholdfromScore."""
    return tuneThresholdfromScore(scores, labels, target_fa, target_fr)


def compute_error_rates(scores, labels):
    """Readable alias for ComputeErrorRates."""
    return ComputeErrorRates(scores, labels)


def compute_min_dcf(fnrs, fprs, thresholds, p_target, c_miss, c_fa):
    """Readable alias for ComputeMinDcf."""
    return ComputeMinDcf(fnrs, fprs, thresholds, p_target, c_miss, c_fa)
